![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)

# Report Management System

A Government Report Management System built using PHP.  
This project allows departments and administrators to manage, track, and review reports efficiently.

---

## Features
- User login and authentication
- Report submission and tracking
- Admin dashboard for managing reports
- Department-wise report categorization
- Simple and clean interface

---

## Installation
1. Clone the repository:
   ```bash
   git clone https://github.com/k-a-r-t-h-i-k-M/report-management-system.git


---
<img width="1917" height="950" alt="Admin 1" src="https://github.com/user-attachments/assets/bddaffb6-239e-4dd9-98df-14cedc60d87a" />
<img width="1919" height="947" alt="Department 1" src="https://github.com/user-attachments/assets/0aed560e-d33e-4a8f-9577-42e7c069e448" />
